...
(defn report-errors [update-results error-output-stream]
  "Writes file update failures to ERROR-OUTPUT-STREAM.
  UPDATE-RESULTS is a seq of UPDATE-SUCCESS records, presumably generated by UPDATE-MTIME."
  (let [failed-updates (filter #(not (:was-successful %)) update-results)
        failure-messages (map #(str (.getPath (:file %)) ": " (:msg %)) failed-updates)
        *out* error-output-stream]
    (println (str/join "\n" failure-messages))))
